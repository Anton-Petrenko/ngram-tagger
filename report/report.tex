...
\input{implementations/anton}
\input{implementations/luis}
\input{implementations/merritt}

\pagebreak
\section{Reflection}

Out of the three different implementations of the base approach, Anton's implementation was the fastest and most accurate. This speed was likely the result of having less overhead by using the built-in Python CSV reading module and storing keys as strings instead of tuples, which take additional time to construct. Luis's approach performed slightly slower than Anton's approach and had slightly lower accuracy, but ran for a consistently short amount of time for all sizes of $n$-gram model. The Merritt's approach performed the slowest by a notable margin, having the most overhead when it came to processing the original data and building the initial dictionary to count occurrences. This was likely due to additional work required by the pandas module when augmenting the data with sentence numbers and history.
